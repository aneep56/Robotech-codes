...
void setup() {
  // Initialize the LEDs
  pinMode(LED_1, OUTPUT); // LED at pin 7
  pinMode(LED_2, OUTPUT); // LED at pin 9
  pinMode(LED_3, OUTPUT); // LED at pin 11
  ...
  // We use INPUT_PULLUP so we don't need a resistor on the switch
...
void loop() {
  if (game_state == 0) {
    ...
    if (digitalRead(FLAP_BUTTON) == LOW) {
      momentum = -4;
      // Beep the buzzer when button is pressed
      beep();
      // Control LED_2 based on button press
    ...
    // add the downward force to the bird position to determine its new position
    bird_y += momentum;
    ...
    // Blink LEDs 7 and 11 simultaneously and beep buzzer for 2 seconds
    long beepDuration = 2000;
    long beepStartTime = millis();
    while (millis() - beepStartTime < beepDuration) {
      digitalWrite(LED_1, HIGH); // LED at pin 7
      digitalWrite(LED_3, HIGH); // LED at pin 11
      digitalWrite(BUZZER_PIN, HIGH); // Beep the buzzer
      delay(100); // Adjust this delay for LED blink rate and buzzer sound duration
    ...
    outlineTextAtCenter(1, "ROBO-TECH");
...
void beep() {
  ...
  delay(100); // Adjust this delay for beep duration
